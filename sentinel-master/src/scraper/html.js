...
async function runHtmlScraper(tierFilter) {
  const registry = loadRegistry();
  let sources = registry.sources.filter(s => s.method === 'html' && s.active === true);
  if (tierFilter) sources = sources.filter(s => s.tier === tierFilter);

  if (sources.length === 0) {
    console.log('[HTML] No active HTML sources found in registry.');
    return;
  }
  ...
  for (const source of sources) {
    console.log(`[HTML] ${source.id} — fetching...`);
    ...
    try {
      ...
      // Adapt selectors to your sources — default looks for article links
      $('a[href]').each((_, el) => {
      ...
      const insertBatch = db.transaction((items) => {
        for (const item of items) {
          ...
          stmts.insertSeenUrl.run(item.url, source.id, ingestedAt);
          newCount++;
        }
      });

      insertBatch(articles.slice(0, 50));

      stmts.insertScrapeLog.run({
        source_id: source.id, run_at: runAt,
        articles_found: articles.length, articles_new: newCount,
      ...
      totalNew += newCount;
      console.log(`[HTML] ${source.id} — ${newCount} new / ${articles.length} total`);

    } catch (err) {
      console.error(`[HTML] ${source.id} — ERROR: ${err.message}`);
      stmts.insertScrapeLog.run({
        source_id: source.id, run_at: runAt,
  ...
  console.log(`[HTML] Run complete. ${totalNew} new articles.`);
  return { totalNew };
}

if (require.main === module) {
  runHtmlScraper().catch(err => {
    console.error('[HTML] Fatal error:', err.message);
    process.exit(1);
  });
}

module.exports = { runHtmlScraper };
